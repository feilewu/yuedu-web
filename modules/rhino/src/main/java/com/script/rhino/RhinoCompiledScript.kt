...
internal class RhinoCompiledScript(
    ...
    override fun eval(scope: Scriptable, coroutineContext: CoroutineContext?): Any? {
        ...
        try {
            ...
            val ret = script.exec(cx, scope as VarScope)
    ...
    override suspend fun evalSuspend(scope: Scriptable): Any? {
        ...
        withContext(RhinoContextElement(cx)) {
            ...
            try {
                cx.checkRecursive()
                try {
                    ret = cx.executeScriptWithContinuations(script, scope as VarScope)
                    ...
                    while (true) {
                        try {
                            ...
                            ret = cx.resumeContinuation(continuation, scope as VarScope, functionResult)
                            break
                        } catch (e: ContinuationPending) {
                            pending = e
                        }

...
object RhinoScriptEngine : AbstractScriptEngine(), Invocable, Compilable {
    ...
    private fun getExecutionScope(scope: Scriptable): VarScope {
        return (scope as? VarScope) ?: (scope.parentScope ?: topLevel!!)
    }
    ...
    override fun eval(
        ...
        try {
            ...
            val jsScope = getExecutionScope(scope)
            ret = script.exec(cx, jsScope, scope)
    ...
    override suspend fun evalSuspend(reader: Reader, scope: Scriptable): Any? {
        ...
        withContext(RhinoContextElement(cx)) {
            ...
            try {
                ...
                val jsScope = getExecutionScope(scope)
                try {
                    ret = cx.executeScriptWithContinuations(script, jsScope)
                    ...
                    while (true) {
                        try {
                            ...
                            ret = cx.resumeContinuation(continuation, jsScope, functionResult)
                            break
                        } catch (e: ContinuationPending) {
                            pending = e
                        }
    ...
    private operator fun invoke(thiz: Any?, name: String?, vararg args: Any?): Any? {
        ...
        try {
            ...
            if (scope == null) {
                scope = engineScope.parentScope ?: topLevel!!
            }
    ...
    override fun getRuntimeScope(context: ScriptContext): Scriptable {
        ...
        val cx = Context.enter()
        try {
            val tl = RhinoTopLevel(cx, this)
            newScope.prototype = tl.globalThis
            newScope.parentScope = tl
        } finally {
            Context.exit()
        }
        return newScope
    ...
    override fun getRuntimeScope(bindings: ScriptBindings): Scriptable {
        val cx = Context.enter()
        try {
            bindings.prototype = cx.initStandardObjects().globalThis
            bindings.parentScope = topLevel
        } finally {
            Context.exit()
        }
        return bindings
    ...
    init {
        ContextFactory.initGlobal(object : ContextFactory() {
            ...
            override fun doTopCall(
                ...
                var current: VarScope? = scope
                while (current != null) {
                    if (current is RhinoTopLevel) {
                        accContext = current.accessContext
                        break
                    }

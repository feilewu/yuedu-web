package io.legado.server.analyzeRule
...
import io.legado.server.constant.AppConst.UA_NAME
import io.legado.server.constant.AppPattern
import io.legado.server.constant.AppPattern.JS_PATTERN
import io.legado.server.constant.AppPattern.dataUriRegex
import io.legado.server.model.entity.BaseSource
import io.legado.server.model.entity.RuleDataInterface
...
import io.legado.server.ConcurrentRateLimiter
import io.legado.server.JsExtensions



import io.legado.server.config.AppConfig
import io.legado.server.http.BackstageWebView
import io.legado.server.http.CookieManager
import io.legado.server.http.CookieManager.mergeCookies
import io.legado.server.http.CookieStore
import io.legado.server.http.RequestMethod
import io.legado.server.http.StrResponse
import io.legado.server.http.addHeaders
import io.legado.server.http.get
import io.legado.server.http.getProxyClient
import io.legado.server.http.newCallResponse
import io.legado.server.http.newCallStrResponse
import io.legado.server.http.postForm
import io.legado.server.http.postJson
import io.legado.server.http.postMultipart
import io.legado.server.source.getShareScope
import io.legado.server.Debug
import io.legado.server.SharedJsScope
import io.legado.server.utils.EncoderUtils
import io.legado.server.utils.GSON
import io.legado.server.utils.GSONStrict
import io.legado.server.utils.NetworkUtils
import io.legado.server.utils.fromJsonArray
import io.legado.server.utils.fromJsonObject

import io.legado.server.utils.isJson
import io.legado.server.utils.isJsonArray
import io.legado.server.utils.isJsonObject
import io.legado.server.utils.isXml
...
class AnalyzeUrl(
    ...
    private fun replaceKeyPageJs() { //先替换内嵌规则再替换页数规则，避免内嵌规则中存在大于小于号时，规则被切错
        // 直接替换 {{key}}, {{page}}, {{baseUrl}} 为实际值
        key?.let {
            ruleUrl = ruleUrl.replace("{{key}}", it)
            ruleUrl = ruleUrl.replace("searchKey", it)
        }
        page?.let {
            ruleUrl = ruleUrl.replace("{{page}}", it.toString())
            ruleUrl = ruleUrl.replace("{{searchPage}}", it.toString())
        }
        if (baseUrl.isNotBlank()) {
            ruleUrl = ruleUrl.replace("{{baseUrl}}", baseUrl)
        }
        //js
    ...
    private fun analyzeUrl() {
        ...
        url = NetworkUtils.getAbsoluteURL(baseUrl, urlNoOption) ?: urlNoOption
    ...
    private fun setCookie() {
        val cookie = kotlin.run {
            ...
            (CacheManager.getFromMemory<String>(key) as Any?)?.let {
                    return@run it
                }
            }
            */
    ...
    private fun saveCookie() {
        ...
        if (enabledCookieJar) {
            val key = "${domain}_cookieJar"
            @Suppress("UNCHECKED_CAST")
            (CacheManager.getFromMemory<String>(key) as? String)?.let {
                if (it is String) {

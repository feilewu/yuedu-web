...
import io.legado.server.model.entity.BaseBook
...
import io.legado.server.model.entity.RssArticle
...
import io.legado.server.utils.getOrPutLimit
import io.legado.server.utils.isDataUrl
import io.legado.server.utils.isJson
import io.legado.server.utils.printOnDebug
import io.legado.server.utils.splitNotBlank
...
class AnalyzeRule(
    ...
    fun getStringList(
        ...
        if (isUrl) {
            ...
            if (result is List<*>) {
                for (url in result) {
                    val absoluteURL = NetworkUtils.getAbsoluteURL(redirectUrl?.toString(), url.toString()) ?: continue
    ...
    fun getString(
        ...
        if (isUrl) {
            return if (str.isBlank()) {
                ...
                NetworkUtils.getAbsoluteURL(redirectUrl?.toString(), str) ?: str
            }
        }
        return str
    }
            ...
            for (sourceRule in ruleList) {
                ...
                result = when (sourceRule.mode) {
                    Mode.Regex -> AnalyzeByRegex.getElement(
                        result.toString(),
                        rule.splitNotBlank("&&").toTypedArray()
                    )
                    Mode.Js -> evalJS(rule, result)
